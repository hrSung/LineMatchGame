...
public abstract class UIRect : MonoBehaviour
{
	...
	void FindCameraFor (AnchorPoint ap)
	{
		...
		else
		{
            ...
            var targetLayer = ap.target.gameObject.layer;
            var uiCams = UICamera.list;
            for (int i = 0; i < uiCams.size; i++)
            {
                if( uiCams[i].gameObject.scene == ap.target.gameObject.scene )
                {
                    var cam = uiCams[i].cachedCamera;
                    if (cam && (cam.cullingMask & 1 << targetLayer) != 0)
                    {
            ...
            {
                ap.targetCam = NGUITools.FindCameraForLayer(targetLayer);
            }
		}
	}
